...
//
// Design: labels are removed in GHL — the placeholder doubles as the
// label, so it must stay clearly legible. Light grey field boxes,
// black typed text, Brandy primary button, Inkwell focus. No
// prefers-color-scheme block (it was flipping fields dark on the
// white card).
...
/* Placeholder = the label now, so keep it clearly legible */
input::placeholder,
...
/* Field icons (e.g. the email envelope) — match placeholder tone */
.input-field svg,
.form-group svg,
.field-container svg {
  color: #5B6470 !important;
